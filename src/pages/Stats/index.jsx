...
function Statistic(props) {
  ...
  const fetchIngredientData = async () => {
    try {
      ...
      setTopIngredients(response.data.top_k_ingredients);
    } catch (error) {
  ...
  const fetchIngredientList = async () => {
    try {
      ...
      const ingredientsList = response.data.map((item) => ({
  ...
  const fetchBeverageList = async () => {
    try {
      ...
      const beverageList = response.data.map((item) => ({
        name: item.Beverage,
      }));
  ...
  const inputBeverage = async () => {
    try {
      await axios.post(`${apiUrl}/drinks`, formData);
